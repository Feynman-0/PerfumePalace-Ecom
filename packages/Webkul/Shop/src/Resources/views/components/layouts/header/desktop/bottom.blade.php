...
<div class="flex min-h-[78px] w-full justify-between border border-b border-l-0 border-r-0 border-t-0 px-[60px] max-1180:px-8">
    ...
    <div class="flex items-center gap-x-10 max-[1180px]:gap-x-5">
        ...
        <a
            ...
            aria-label="Perfume Palace - Luxury Fragrances"
            class="logo-container"
        >
            @php
                $channelLogo = core()->getCurrentChannel()->logo ?? null;
            @endphp
            
            @if ($channelLogo)
                <img
                    src="{{ Storage::url($channelLogo) }}"
                    alt="Perfume Palace"
                    style="max-height: 65px; width: auto; height: auto;"
                    onerror="this.style.display='none'; this.nextElementSibling.style.display='block';"
                >
            @endif
            <span class="logo-text" style="{{ $channelLogo ? 'display:none;' : '' }}">
                Perfume Palace
            </span>
        </a>
