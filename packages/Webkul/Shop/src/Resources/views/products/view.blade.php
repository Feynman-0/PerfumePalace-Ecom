...
<x-shop::layouts>
    ...
    {!! view_render_event('bagisto.shop.products.view.slider.before', ['product' => $product]) !!}

    <!-- Homepage Slider -->
    @php
        $sliderCustomization = \Webkul\Theme\Repositories\ThemeCustomizationRepository::class;
        $sliderData = app($sliderCustomization)->findOneWhere(['type' => 'image_carousel', 'status' => 1, 'channel_id' => core()->getCurrentChannel()->id]);
    @endphp

    @if($sliderData && $sliderData->options)
        <x-shop::carousel
    ...
    {!! view_render_event('bagisto.shop.products.view.slider.after', ['product' => $product]) !!}
